...
async def remove_user_saved_episodes_tool(ctx: Context[ServerSession, AppContext], episode_ids: list[str]) -> str:
    ...
    await remove_user_saved_episodes(spotantic_client, episode_ids=episode_ids)
    return f"Removed {len(episode_ids)} episode(s) from the user's library."
