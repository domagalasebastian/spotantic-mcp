...
async def get_current_user_playlist_tool(
    ctx: Context[ServerSession, AppContext],
    *,
    limit: int = 10,
    offset: int = 0,
) -> PagedResultView[SimplifiedPlaylistView]:
    ...
    Returns:
        A paged result containing playlists owned or followed by the current Spotify user.
    ...
    api_data = (
        await get_current_user_playlist(
            spotantic_client,
            limit=limit,
            offset=offset,
        )
    ...
    return PagedResultView[SimplifiedPlaylistView].model_validate(api_data)
