...
async def get_user_saved_albums_tool(
    ctx: Context[ServerSession, AppContext], limit: int = 10, offset: int = 0, market: str | None = None
) -> PagedResultView[SavedAlbumView]:
    """Get a list of the albums saved in the current Spotify user's 'Your Music' library.

    Use pagination (limit/offset) to fetch albums in smaller chunks to minimize response size.
    ...
    Returns:
        A paged result containing saved album objects for the albums in the user's library.
    ...
    return PagedResultView[SavedAlbumView].model_validate(api_data)
