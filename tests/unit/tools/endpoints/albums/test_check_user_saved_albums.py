...
from spotantic_mcp.tools.endpoints.albums._check_user_saved_albums import check_user_saved_albums_tool


@pytest.mark.asyncio
async def test_check_user_saved_albums_tool(mock_context):
    mock_response = MagicMock()
    mock_response.data = {"id1": True}

    with patch(
        "spotantic_mcp.tools.endpoints.albums._check_user_saved_albums.check_user_saved_albums",
        new=AsyncMock(return_value=mock_response),
    ) as mock_api_call:
        res = await check_user_saved_albums_tool(ctx=mock_context, album_ids=["123"])
